...
### **ir_builder.py** - IR構築
- .ksスクリプトをIR（中間表現）に変換するビルダー
- スクリプト解析結果を構造化されたIRオブジェクト列に変換

### **ir_model.py** - IRモデル定義
- 中間表現（IR）のデータクラス・型定義
- コマンド・テキスト・分岐などの構造を定義

### **inline_markup.py** - インラインマークアップ
- ルビ（振り仮名）・傍点などのインライン装飾を処理
- テキスト内の特殊タグを解析して描画データに変換

### **dialogue_subsystem.py** - ダイアログサブシステム
- 対話システム全体をサブシステムクラスとして管理
- 初期化・更新・描画の統一インターフェースを提供
