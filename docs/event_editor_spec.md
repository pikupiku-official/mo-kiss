...
- BGM/SEの `範囲編集…` → 音源の実尺を取得したポップアップで開始・終了を2ハンドル編集し、選択範囲を再生しながらトリミング。BGMは `start` / `end`、SEは `start` / `end` として保存
- BGM/SEの `volume` → 0.0〜2.0（最大200%）のフェーダーを主操作とし、小型の数値欄も併設。2.0未満は通常ゲイン、2.0までの増幅はピーク制限して音割れを避ける
