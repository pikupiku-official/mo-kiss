...
### 飛ばし処理（sequence）
- **complete**: 最終状態まで100msで超高速補間
- **interrupt**: 最終状態へ100msクロスフェード
  ...
  "actions": [
    {
      ...
      "params": {
        "time": 500,
        "to": [-0.4, 0.0],
        "zoom": 2.0 //ここはドキドキスクリプトの問題で、chara_show時の画像の拡大表示を維持するために重複して指定されている。ドキドキスクリプト側の仕様変更で消すべき。zoomタグに関しては、現在のサイズから更に何倍するか、という拡大縮小アニメーションが伴う時にだけ指定する。
      },
      "animation": {
        "type": "once",
        "on_advance": "complete"
