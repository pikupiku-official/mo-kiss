...
mo-kiss/
├── main.py                 # エントリーポイント
├── event_editor.py        # イベントエディタ（Windows用）
...
├── images/                # 画像リソース
├── sounds/                # 音声リソース
├── events/                # イベントデータ（.ksファイル、events.csv）
├── tools/                 # 開発ツール
│   ├── preview_dialogue.py  # ダイアログプレビュー
│   └── notify_deadlines.py  # 締め切り通知
├── dialogue/              # 会話システム
...
- [ ] `core.config` から設定を読み込んでいるか（`from core.config import ...` または `from .config import ...`）
- [ ] スケール係数を適用しているか
- [ ] 4:3コンテンツ領域を考慮しているか
...
- 2026-05-14: `config.py` → `core/config.py` 移動
- 2026-05-14: `character_gender.json` → `data/character_gender.json` 移動
- 2026-05-14: `claude.md` → `docs/claude.md` 移動（本ファイル）
