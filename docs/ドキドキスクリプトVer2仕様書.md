...
CGは通常のキャラクター表示を一時的に覆い、`cg_hide` で元のキャラクター
状態へ戻します。画像IDは `MMK_nn_nnn` 全体を `storage` に指定します。
`nn` がCG番号、`nnn` が差分番号です。
...
- `storage`: CG画像ID（必須）
- `fade` または `time`: クロスフェード時間（秒、デフォルト 0.3）
- 720×480のCGは仮想画面の高さへ合わせ、左右を中央から切り取って表示

### CG差分・演出変更

```
[cg_shift storage="MMK_03_001" left="0.02" top="0" zoom="1.1" time="600" fade="0.3"]
```

- `storage`: 差分画像ID。省略時は現在の画像を維持
- `left` / `top`: 現在位置からの相対移動量（仮想画面比）
- `zoom`: 目標ズーム倍率（省略時は現在値）
- `time`: 移動時間（ミリ秒、デフォルト 600）
- `fade`: `storage` を変更する場合のクロスフェード時間（秒）
- 移動と差分切替は同時に実行し、遷移中は次のstepへ進めません
...
CGの非表示完了後、CG表示前から保持していたキャラクター状態を再表示します。
CG表示中の `chara_show` / `chara_shift` は論理状態だけ更新し、CGの上には描画しません。
...
**パラメータ**
- `bgm`: BGMファイル名
- `volume`: 0.0〜2.0（省略時は設定値。2.0までの増幅はピーク制限）
- `start` / `end`: 再生範囲（秒、省略時は音源全体）
