            ...
            try:
                ...
                # BGM設定を検出
                    ...
                    try:
                        ...
                        if bgm_parts and bgm_volume and bgm_loop:
                            current_bgm = bgm_parts.group(1)
                            current_bgm_volume = bgm_volume.group(1)
                            current_bgm_loop = bgm_loop.group(1)
                            if self.debug:
                ...
                # セリフを検出
                    ...
                    try:
                        dialogue_text = re.search(r'「"([^"]+)"」', line)
                        if dialogue_text:
                            dialogue_text = dialogue_text.group(1)
                            if self.debug:
                ...
                # キャラクター移動コマンドを検出
                    ...
                    try:
                        name_parts = re.search(r'sub="([^"]+)"', line)
                        time = re.search(r'time="([^"]+)"', line)
                        left = re.search(r'left="([^"]+)"', line)
                        top = re.search(r'top="([^"]+)"', line)
                        if name_parts and left and top:
                            char_name = name_parts.group(1)
