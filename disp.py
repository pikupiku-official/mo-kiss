...
# Pygameを初期化
pygame.init()
pygame.mixer.init()  # サウンドモジュールの初期化
...
# BGMの設定
BGM_PATH = os.path.join("sounds", "bgms")
DEFAULT_BGM = "maou_bgm_8bit29.mp3"
SECOND_BGM = "maou_bgm_piano41.mp3"

# BGMを再生する関数
def play_bgm(filename, volume=0.3):
    try:
        bgm_path = os.path.join(BGM_PATH, filename)
...
# TyranoScriptファイルから会話データを読み込む関数
def load_dialogue_from_ks(filename):
    try:
        with open(filename, 'r', encoding='utf-8') as file:
            content = file.read()

        dialogue_list = []
        current_background = "school"  # デフォルトの背景
        current_character = "girl1"    # デフォルトのキャラクター
        current_eye = "eye1"          # デフォルトの目
        current_mouth = "mouth1"      # デフォルトの口
        current_bgm = DEFAULT_BGM     # デフォルトのBGM

        # 背景設定を検出
        bg_matches = re.finditer(r'\[BGM bgm="([^"]+)"\]', content)
        for match in bg_matches:
            bgm_name = match.group(1)
            if bgm_name == "classroom":
                current_bgm = SECOND_BGM
            else:
                current_bgm = DEFAULT_BGM

        # キャラクターとテキストを検出
        text_matches = re.finditer(r'\[(GIRL\d+)\]([^[]+)\[en\]', content)
...
# 会話データをTyranoScriptファイルから読み込む
try:
    dialogue_data = load_dialogue_from_ks("dialogue.ks")
except Exception as e:
...
if not dialogue_data:
    dialogue_data = [
        ["classroom", "girl1", "eye1", "mouth1", "TyranoScriptファイルが読み込めませんでした。", DEFAULT_BGM],
        ["classroom", "girl1", "eye1", "mouth1", "デフォルトの会話を表示しています。", DEFAULT_BGM]
    ]
...
text_font = pygame.font.SysFont('M PLUS ROUNDED', 20)
...
# 最初のBGMを再生
if dialogue_data:
    play_bgm(dialogue_data[0][5])  # 最初の会話のBGMを再生
...
while running:
    ...
    if current_dialogue:
        bg_name, char_name, eye_type, mouth_type, dialogue_text, bgm_name = current_dialogue
