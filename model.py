...
def initialize_first_scene(game_state):
    ...
    if game_state['dialogue_data']:
        ...
        if len(current_dialogue) >= 6:
            ...
            # テキストの有効性をチェック
            if not dialogue_text or dialogue_text.startswith("_MOVE_"):
                # 移動コマンドまたは無効なテキストの場合、次の有効な対話に進む
...
def advance_dialogue(game_state):
    """次の対話に進む"""
    if game_state['current_paragraph'] >= len(game_state['dialogue_data']) - 1: #消去予定
        if DEBUG:
            print("対話の終了に達しました")
        return False
